...
export default class BaseLogger {
    ...
    _report(dataSource, reporterEndpoint) {
    ...
    _handleBlackboxPageTransitionMeasurement(pageMeasurement) {
        this._sendBlackboxMeasurement(pageMeasurement);
    }

    _handleBlackboxPerformance(perfEventData) {
        const actualEnv = env();
        const {
            wixPerformanceMeasurements
        } = actualEnv;

        const isPerformanceMeasurements = perfEventData && perfEventData.length;
        const isPageTransitionMeasurement =
            perfEventData && perfEventData.entryType;

        if (isPerformanceMeasurements) {
            this._handleBlackboxPerformanceEntries(perfEventData);
        } else if (isPageTransitionMeasurement) {
            this._handleBlackboxPageTransitionMeasurement(perfEventData);
        } else {
            if (actualEnv.addEventListener) {
                actualEnv.addEventListener('wixPerformanceMeasurements', (event) =>
                    this._handleBlackboxPerformance(event.detail),
                );
                actualEnv.addEventListener('wixPageMeasurements', (event) =>
                    this._handleBlackboxPerformance(event.detail),
                );
            }
    ...
    _handleBlackboxPerformanceEntries(measurements) {
        measurements.forEach((performanceEntryPromise) => {
            performanceEntryPromise.then &&
